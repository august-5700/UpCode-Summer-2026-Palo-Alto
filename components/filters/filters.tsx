...
export default function Filters() {
  ...
  // when collapsed: just the glass filter button
  if (!open) {
    return (
      <button
        type="button"
        onClick={() => setOpen(true)}
        className="absolute top-4 left-72 z-[1000] flex h-11 w-11 items-center justify-center rounded-full border border-white/40 bg-white/50 shadow-lg backdrop-blur-2xl backdrop-saturate-150 transition hover:bg-white/70"
      >
  ...
  // expanded: the same icon becomes the panel header the dialogue comes out from it
  return (
    <div className="absolute top-4 left-72 z-[1000] w-[440px] rounded-3xl border border-white/40 bg-white/50 p-5 shadow-2xl backdrop-blur-2xl backdrop-saturate-150">
      <div className="mb-5 flex items-center gap-3">
        <button
          type="button"
          onClick={() => setOpen(false)}
          className="flex h-11 w-11 shrink-0 items-center justify-center rounded-full bg-white/80 shadow transition hover:bg-white"
        >
          <Filter className="h-5 w-5 text-gray-800" />
        </button>
        <h2 className="text-xl font-bold tracking-tight text-gray-900">Filter (by minimum)</h2>
      </div>
